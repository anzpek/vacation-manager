    ...
    setupModals: function() {
        ...
        if (resetFilterBtn) {
            resetFilterBtn.addEventListener('click', () => {
                document.querySelectorAll('#teamFilterCheckboxes input').forEach(cb => cb.checked = false);
                ...
                Object.keys(departmentGroups).forEach(deptId => {
                    ...
                    html += `
                        <div class="department-filter-group">
                            <div class="department-header" onclick="vacationManager.toggleDepartmentFilter('${deptId}')">
                                <span class="folder-icon">📁</span>
                                <label>
                                    <input type="checkbox" class="dept-all-checkbox" data-dept="${deptId}">
                                    <strong>🏢 ${dept.name} (${deptEmployees.length}명)</strong>
                                </label>
                            </div>
                            <div class="department-content" id="dept-${deptId}" style="display: none; margin-left: 20px;">
                    `;
                    
                    // 부장 표시
                    if (dept.manager) {
                        html += `
                            <label class="employee-filter manager">
                                <input type="checkbox" name="employeeFilter" value="${dept.manager.name}" class="employee-checkbox" data-dept="${deptId}">
                        ...
                        if (teamEmployees.length > 0) {
                            html += `
                                <div class="team-filter-group" style="margin-left: 10px;">
                                    <div class="team-header" onclick="vacationManager.toggleTeamFilter('${deptId}', '${teamId}')">
                                        <span class="folder-icon">📂</span>
                                        <label>
                                            <input type="checkbox" class="team-all-checkbox" data-dept="${deptId}" data-team="${teamId}">
                                            <strong>🛠️ ${team.name} (${teamEmployees.length}명)</strong>
                                        </label>
                                    </div>
                                    <div class="team-content" id="team-${deptId}-${teamId}" style="display: none; margin-left: 20px;">
                            `;
                            
                            // 팀장 먼저 표시
                            if (team.leader) {
                                html += `
                                    <label class="employee-filter leader">
                                        <input type="checkbox" name="employeeFilter" value="${team.leader.name}" class="employee-checkbox" data-dept="${deptId}" data-team="${teamId}">
                                        🥈 ${team.leader.name} (팀장)
                                    </label>
                                `;
                            }
                            ...
                            team.members.sort((a, b) => a.name.localeCompare(b.name)).forEach(emp => {
                                html += `
                                    <label class="employee-filter member">
                                        <input type="checkbox" name="employeeFilter" value="${emp.name}" class="employee-checkbox" data-dept="${deptId}" data-team="${teamId}">
                                        👤 ${emp.name}
                                    </label>
                                `;
                            });
